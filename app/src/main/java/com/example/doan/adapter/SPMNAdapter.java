...
public class SPMNAdapter extends RecyclerView.Adapter<SPMNAdapter.ItemHolder> {
    ...
    public void onBindViewHolder(@NonNull ItemHolder holder, int position) {
        ...
        if(sp.getPromotion() > 0){
            holder.txtGiaSP.setText("Giá : " + format.format(Double.valueOf(sp.getPrice() - (sp.getPrice()*sp.getPromotion()/100))) + " $" + " (-" + sp.getPromotion() + "%)" + "(SL:" + sp.getQuantity() + ")");
        }else{
            holder.txtGiaSP.setText("Giá : " + format.format(Double.valueOf(sp.getPrice())));
        }
